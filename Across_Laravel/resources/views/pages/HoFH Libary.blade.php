            ...
            <ul class="list-unstyled components">
                ...
                <li>
                    ...
                    <ul class="collapse list-unstyled" id="pageSubmenu">
                        <li>
                            <a   href="{{ route('secondcategory.create') }} ">New Category</a>
                        </li>
                        <li>
                                    ...
                                    @if($item->parent_id == null)
                                        <tr>
                                            ...
                                            <td class="table-action" style=" "><a href="/categories/{{$item->id}}/edit" class="edit"  > <i class="far fa-edit"></i></a>
                                                <form method="POST" action="/categories/{{$item->id}} " onsubmit="return confirm('Are you sure ?')" class="TableForm">
                                                    {{method_field('delete')}}
                                                    {!! csrf_field() !!}
                                                    <button type="submit" style="border: none; background: none; cursor: pointer" ><i class="fas fa-trash-alt"></i></button>
                                                </form>
                                            </td>
                                            <td><a href="{{route('secondcategory.create', ['cid' => $item->id])}}" ><i class="fas fa-plus"></i></a></td>
                                            <td>
                                        </tr>

                                        @foreach($posts as $post)
                                            @if($post->post_parent_id == $item->id)
                                                <tr class="collapse list-unstyled" id="sub{{$item->id}}" style="background: #6cb2eb; margin-left: 10px">
                                                    ...
                                                    <td class="table-action" style="display: inline-flex;">
                                                        <a href="/posts/{{$post->id}}/edit" class="edit" >  <i class="fas fa-user-edit"></i></a>
                                        ...
                                        @foreach($sub_category as $child)
                                            @if($child->parent_id == $item->id)
                                                ...
                                                <tr class="collapse list-unstyled" id="sub{{$item->id}}" style="background: honeydew;">
                                                    ...
                                                    <td>{{$child->name}} </td>
                                                    <td class="table-action " >
                                                        <a href="/categories/{{$child->id}}/edit" class="edit" > <i class="far fa-edit"></i></a>
                                                    ...
                                                    <td><a href="{{route('posts.create', ['cid' => $child->id])}}" ><i class="fas fa-plus"></i></a></td>
                                                </tr>
                                                @foreach($posts as $post)
                                                    @if($post->post_parent_id == $child->id)
                                                        <tr class="collapse list-unstyled" id="post{{$child->id}}" style="background: #6cb2eb;">
                                                            <td class="table-user">{{$post->title}}</td>
                                                            <td>{{$post->date}}</td>
                                                            <td>{{$post->posted_by}}</td>
                                                            <td>{{$post->language}}</td>
